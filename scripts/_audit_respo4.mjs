...
const PORT=4442, DIST='/home/user/openFermentUI/dist';
...
const R = await page.evaluate(()=>{
  ...
  function pth(el){const a=[];let n=el;while(n&&n!==document.documentElement&&a.length<7){a.unshift(sel(n));n=n.parentElement}return a.join(' > ')}
  const outs=[];
  for(const el of document.querySelectorAll('body *')){const cs=getComputedStyle(el);if(cs.display==='none')continue;const r=el.getBoundingClientRect();if(r.right>421 && (cs.position==='absolute'||cs.position==='fixed'||cs.position==='sticky')) outs.push({sel:pth(el),right:Math.round(r.right),pos:cs.position});}
  return {windowScrollX:sx, deSW:document.documentElement.scrollWidth, positioned:outs.slice(0,10)};
});
