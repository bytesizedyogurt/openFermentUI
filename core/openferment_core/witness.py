...
def _gather() -> tuple[list[Candidate], list[DroppedCandidate], set[str]]:
    ...
    candidates = [c for r in responses for c in r.candidates]
...
def runs() -> list[ExtractRun]:
    ...
    the seed. Empty when nothing has been extracted: no run, no number."""
    ...
    log.info(
        "witness: %s over %d papers → %d scored (%s), %d new",
        RUN,
        len(papers),
        len(run.results),
...
def new_candidates() -> list[Candidate]:
    """The unscored candidates, for `overlay.candidates` (§2.1, §7.3)."""
    candidates, _, papers = _gather()
    if not papers:
        return []
    return new_records(candidates, load_corpus().records, papers=papers)
