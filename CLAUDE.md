...
- **`pnpm verify` runs with no key, no service and no network, and must stay
  green.** Fifteen stages, in order: check:secrets → typecheck → check:plan →
  check:reference → test:core → check:seed → check:lock → check:capture →
  build → test:durable → test:deposition → test:reconcile → test:smoke →
  test:golden → test:deep. Live tests are `pnpm test:live` and never in verify.
